...
//开始界面 显示名称 班级 项目
void oled_start_init(){
	OLED_NewFrame();
		for(uint8_t i = 0 ; i <120; i++){
		OLED_PrintString( i - 100, 10, "2301 陈宇杰", &font16x16 , OLED_COLOR_NORMAL);
		OLED_PrintString( i - 120, 30, "====车载大灯====", &font16x16,  OLED_COLOR_NORMAL);
		OLED_ShowFrame();
		HAL_Delay(10);
		}
	HAL_Delay(300);
	OLED_NewFrame();
		for(uint8_t i = 0 ; i <120; i++){
		 OLED_PrintString( i - 118, 10, "KEY1:自动大灯", &font16x16 , OLED_COLOR_NORMAL);
		 OLED_PrintString( i - 118, 25, "KEY2:自动远近光", &font16x16,  OLED_COLOR_NORMAL);
		 OLED_PrintString( i - 118, 40, "KEY3:转向辅助", &font16x16,  OLED_COLOR_NORMAL);
		 OLED_ShowFrame();
		 HAL_Delay(10);
		}
}
