...
**时序要求**：
- **0 码**：高电平 ≤ 0.4μs，低电平 ≥ 0.85μs（占空比 ~30%）
- **1 码**：高电平 ≥ 0.85μs，低电平 ≤ 0.4μs（占空比 ~70%）
- **Reset 码**：低电平 ≥ 125μs

**为什么用 PWM + DMA**：
- 灯带对时序要求严格，GPIO 翻转方式会频繁打断 CPU
- PWM + DMA 方式：CPU 只需准备颜色数据，硬件自动完成精确时序

**定时器配置**：
- 时钟频率：72MHz
- 预分频：90-1 → 72MHz / 90 = 800KHz（周期 1.25μs）
- 自动重装载：无需额外配置
- DMA 触发条件：计数器值与比较寄存器值相等时触发 DMA 事件

**比较值定义**：
```c
#define CODE0  30  // 高电平约 0.4μs（0 码）
#define CODE1  60  // 高电平约 0.8μs（1 码）
```
